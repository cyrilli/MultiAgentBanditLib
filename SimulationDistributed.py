...
if __name__ == '__main__':
	...
	if global_dim == context_dimension:
		users = UM.simulateThetaForHomoUsers()
	else:
		users = UM.simulateThetaForHeteroUsers(global_dim=global_dim)
	AM = ArticleManager(context_dimension, n_articles=n_articles, FeatureFunc=gaussianFeature, argv={'l2_limit': 1})
	...
	algorithms['DisLinUCB'] = DisLinUCB(dimension=context_dimension, alpha=-1, lambda_=lambda_,
												   delta_=delta,
												   NoiseScale=NoiseScale, threshold=D2)
	algorithms['AsyncLinUCB'] = AsyncLinUCB(dimension=context_dimension, alpha=-1, lambda_=lambda_, delta_=delta,
												   NoiseScale=NoiseScale, gammaU=5, gammaD=5)
	algorithms['AsyncLinUCB_AM'] = AsyncLinUCB_AM(dimension_g=global_dim, dimension_l=context_dimension - global_dim, alpha=-1,
											lambda_=lambda_,
											delta_=delta,
											NoiseScale=NoiseScale, gammaU=5, gammaD=5)
	algorithms['FedGLB_UCB'] = FedGLB_UCB(dimension=context_dimension, lambda_=lambda_, threshold=D2, n_users=n_users, c_mu=c_mu, delta=delta, S=S, R=R, alpha=None, alpha_t_scaling=0.1, init_x=np.zeros(context_dimension), max_iters=None)
	## Run Simulation ##
